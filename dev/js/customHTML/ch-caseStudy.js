class CaseStudy extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.css.textContent = '.wrapper{position: relative; width: 100%; height: 200px; border: 1px solid #333; margin: 200px 0; cursor: pointer; transition: background .2s, border .2s}' +
            '.wrapper:hover {border: 1px solid white; background: #151515; transition: background .2s, border .2s}' +
            '.img-wrapper {position: absolute; background: #a73030; top: -100px; width: 250px; height: 400px; box-shadow: 0 0 20px rgba(0, 0, 0, .4); transition: transform .2s; overflow: hidden; display: flex; justify-content: center; align-items: center}' +
            '.right {right: 0px !important; left: auto;}' +
            'img{width: auto; height: 400px; transition: transform .2s ease-out}' +
            '.img-wrapper:hover img {transform: scale3d(0.8, 0.8, 1); transition: transform .2s ease-out}' +
